...
<head>
    ...
    <title>Tes Dengar Lansia - Petugas</title>
...
<body>
    <div x-data="appData()" x-init="init()" class="max-w-4xl mx-auto p-6">
        ...
        <div class="flex justify-between items-center mb-6 mt-8">
            <h1 class="text-3xl text-center font-bold">Tes Pendengaran Lansia</h1>
        ...
        function appData() {
            return {
                ...
                updatePesertaList() {
                    ...
                    // Urutkan peserta berdasarkan submitdengar
                    const sortedPeserta = this.filteredPeserta().sort((a, b) => {
                        ...
                        const submitdengarA = dataKesehatanA ? dataKesehatanA.submitdengar : 0;
                        const submitdengarB = dataKesehatanB ? dataKesehatanB.submitdengar : 0;
    
                        if (submitdengarA === 0 && submitdengarB !== 0) return -1;
                        if (submitdengarB === 0 && submitdengarA !== 0) return 1;
                    ...
                    sortedPeserta.forEach(item => {
                        ...
                        card.classList.add(dataKesehatan && dataKesehatan.submitdengar > 0 ? 'bg-gray-200' : 'bg-white');
                        ...
                        // Kondisi pendengaran
                        const pendengaranText = document.createElement('p');
                        pendengaranText.classList.add('text-sm', 'text-gray-600', 'mb-2');
                        pendengaranText.textContent = dataKesehatan && dataKesehatan.dengar
                            ? 'Terdapat penurunan pendengaran'
                            : 'Kondisi pendengaran baik';
                        contentLeft.appendChild(pendengaranText);
    
                        // Link kuisioner
                        if (dataKesehatan) {
                            const questionnaireLink = document.createElement('a');
                            questionnaireLink.href = `/kuisioner_dengar/${dataKesehatan.id}/${dataKesehatan.jadwal_id}/${this.userId}`;
