    ...
    <div class="max-w-4xl mx-auto p-6">
        ...
        $sortedPeserta = $peserta->sortBy(function ($item) use ($jadwalId) {
            ...
            if (isset($dataKesehatan)) {
                $isComplete = $dataKesehatan->submitmobilisasi;
        ...
        <div id="pesertaList">
            @foreach($sortedPeserta as $index => $item)
                ...
                <div class="card mb-6 p-4 rounded-lg {{ isset($dataKesehatan) && $dataKesehatan->submitmobilisasi ? 'bg-gray-200' : 'bg-white' }} flex justify-between items-center">
                    <div>
                        <h2 class="text-xl font-bold">{{ $item->nama_peserta_lansia }}</h2>
                        <p class="text-sm text-gray-600">
                            @if (isset($dataKesehatan) && $dataKesehatan->mobilisasi)
                                Terdapat penurunan mobilisasi
                            @else
                                Kondisi mobilisasi baik
                            @endif
                        </p>
                    </div>

                    @if(isset($dataKesehatan))
                        <a href="/kuisioner_mobilisasi/{{ $dataKesehatan->id }}/{{ $dataKesehatan->jadwal_id }}"
