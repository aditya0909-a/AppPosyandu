...
<head>
    ...
    <title>Tes Melihat Lansia - Petugas</title>
...
<body>
    <div x-data="appData()" x-init="init()" class="max-w-4xl mx-auto p-6">
        ...
        <div class="flex justify-between items-center mb-6 mt-8">
            <h1 class="text-3xl text-center font-bold">Tes Penglihatan Lansia</h1>
        </div>
        ...
        function appData() {
            return {
                ...
                updatePesertaList() {
                    ...
                    // Urutkan peserta berdasarkan submitlihat
                    const sortedPeserta = this.filteredPeserta().sort((a, b) => {
                        ...
                        const submitlihatA = dataKesehatanA ? dataKesehatanA.submitlihat : 0;
                        const submitlihatB = dataKesehatanB ? dataKesehatanB.submitlihat : 0;
    
                        if (submitlihatA === 0 && submitlihatB !== 0) return -1;
                        if (submitlihatB === 0 && submitlihatA !== 0) return 1;
                    ...
                    sortedPeserta.forEach(item => {
                        ...
                        card.classList.add(dataKesehatan && dataKesehatan.submitlihat > 0 ? 'bg-gray-200' : 'bg-white');
                        ...
                        // Kondisi penglihatan
                        const penglihatanText = document.createElement('p');
                        penglihatanText.classList.add('text-sm', 'text-gray-600', 'mb-2');
                        penglihatanText.textContent = dataKesehatan && dataKesehatan.lihat1 || dataKesehatan.lihat2
                            ? 'Terdapat penurunan penglihatan'
                            : 'Kondisi penglihatan baik';
                        contentLeft.appendChild(penglihatanText);
    
                        // Link kuisioner
                        if (dataKesehatan) {
                            const questionnaireLink = document.createElement('a');
                            questionnaireLink.href = `/kuisioner_lihat/${dataKesehatan.id}/${dataKesehatan.jadwal_id}/${this.userId}`;
