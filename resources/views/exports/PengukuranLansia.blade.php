        ...
        <tbody>
            @foreach ($data as $index => $peserta)
                <tr>
                    ...
                    <td>{{ $peserta->NIK_lansia }}</td>
                    <!-- Menampilkan data kesehatan jika ada -->
                    <td>{{ $peserta->dataKesehatan->isNotEmpty() ? $peserta->dataKesehatan->first()->tinggi_lansia : 'N/A' }}</td>
                    <td>{{ $peserta->dataKesehatan->isNotEmpty() ? $peserta->dataKesehatan->first()->berat_lansia : 'N/A' }}</td>
                    <td>{{ $peserta->dataKesehatan->isNotEmpty() ? $peserta->dataKesehatan->first()->lingkar_lengan_lansia : 'N/A' }}</td>
                    <td>{{ $peserta->dataKesehatan->isNotEmpty() ? $peserta->dataKesehatan->first()->lingkar_perut_lansia : 'N/A' }}</td>
                </tr>
            @endforeach
        </tbody>
    </table>
